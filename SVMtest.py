...
# we can use this for development so our code runs faster.                       ????：dev
num_training = 49000
num_validation = 1000   #验证集
num_test = 1000
num_dev = 500
...
mask = np.random.choice(num_training, num_dev, replace=False)   #返回一向量，从0 - num_training随机选num_dev个数
X_dev = X_train[mask]
y_dev = y_train[mask]
...
# Preprocessing: subtract the mean image  - 减去图片均值
# first: compute the image mean based on the training data
mean_image = np.mean(X_train, axis=0)   #训练集均值-行向量
...
X_train = np.hstack([X_train, np.ones((X_train.shape[0], 1))])  #hstack((a,b))将b附在a后
...
W = np.random.randn(3073, 10) * 0.0001   #np.random.randn(3073, 10)产生(3073, 10)大小数组，元素服从N(0,1)

loss, grad = svm_loss_naive(W, X_dev, y_dev, 0.000005)  # 0.000005                                              ？？？
...
for lr in learning_rates:
    for rs in regularization_strengths:
        ...
        results[(lr, rs)] = (acc_train, acc_val)#result为字典，使用元组作为键，元组作为值
        if best_val < acc_val:
            best_val = acc_val
            best_svm = svm
        # Print out results.
